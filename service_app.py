...
2026 - Seasonal Customer Targeting
...
MONTH_THEMES = {
    ...
    6: {"theme": "Peak Summer Demand", "push": "June is the busiest month for service. Get customers on the schedule now before bays fill up. PM intervals are hitting hard."},
    7: {"theme": "Mid-Season Maintenance", "push": "Machines have been running hard for two months. Target customers due for mid-season service, inspections, and wear item replacement."}
}
...
def show_admin_dashboard():
    ...
    <div class="header-bar">
        <span class="branch-name">Admin Dashboard - 2026 Service Campaign</span>
...
def show_dashboard():
    ...
    # Filter to branch AND month with batch handling
    all_month_data = data[(data['Branch'] == branch_id) & (data['Month'] == month_id)].copy()
    
    if 'Batch' in all_month_data.columns and all_month_data['Batch'].notna().any():
        batch1 = all_month_data[all_month_data['Batch'] == 1]
        batch2 = all_month_data[all_month_data['Batch'] == 2]
    else:
        batch1 = all_month_data
        batch2 = pd.DataFrame()
    
    call_log_check = st.session_state.call_log
    batch1_called_count = sum(
        1 for _, row in batch1.iterrows()
        if call_log_check.get(f"{row['Customer']}_{month_id}", {}).get('called', False)
    )
    batch1_total = len(batch1)
    batch2_unlocked = batch1_called_count >= batch1_total and batch1_total > 0 and len(batch2) > 0
    ...
    if len(batch2) > 0:
        if batch2_unlocked:
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, #1a5c2a, #2d8a4e); color: white; 
                        padding: 12px 20px; border-radius: 8px; margin: 10px 0; text-align: center;">
                <strong>Batch 2 Unlocked!</strong> You called all {batch1_total} Batch 1 customers. 
                {len(batch2)} bonus customers added to your list.
            </div>
            """, unsafe_allow_html=True)
        else:
            remaining = batch1_total - batch1_called_count
            pct = batch1_called_count / batch1_total * 100 if batch1_total > 0 else 0
            st.progress(pct / 100, text=f"Batch 1: {batch1_called_count}/{batch1_total} called — {remaining} more to unlock {len(batch2)} bonus customers")
    ...
    <div class="info-banner">
        <strong>{month_name} Campaign: {campaign_theme}</strong> &bull; {len(df)} targeted customers &bull; {strong_ct} proven multi-year buyers<br>
        {campaign_push}<br>
        Use the customer history and equipment info below to tailor your outreach.
    </div>
    ...
    filtered = filtered.sort_values(['Called', '_ts', 'TotalRevenue'], ascending=[True, True, False])
    ...
    for _, row in filtered.iterrows():
        ...
        # History
        with c5:
            t_rev = row['TotalRevenue'] if pd.notna(row['TotalRevenue']) else 0
            ...
            rev_str = f"${t_rev:,.0f} total spend"
    ...
    <div style="text-align: center; color: #999; font-size: 11px; padding: 20px 0; border-top: 1px solid #eee;">
        Created by Nick Butler &bull; Southeastern Equipment &bull; 2026
    </div>
    """, unsafe_allow_html=True)
